...
def run(img):
    ...
    #(Devide the image into three parts: top, middle, and bottom. This allows faster and more accurate processing.)
    for r in roi:
        roi_h = roi_h_list[n]
        n += 1       
        ...
        if cnt_large is not None:#(if the contour is not empty)
            rect = cv2.minAreaRect(cnt_large)#(the minimum bounding rectangle)
            box = np.intp(cv2.boxPoints(rect))#(four vertex the minimum bounding rectangle)
            for i in range(4):
            ...
            #(obtain the diagonal point of the rectangle)
            pt1_x, pt1_y = box[0, 0], box[0, 1]
            pt3_x, pt3_y = box[2, 0], box[2, 1]            
            center_x, center_y = (pt1_x + pt3_x) / 2, (pt1_y + pt3_y) / 2#(ceter point)
            cv2.circle(img, (int(center_x), int(center_y)), 5, (0,0,255), -1)#(draw the center point)
            center_.append([center_x, center_y])                        
            #(sum the three center points of the top, middle, and bottom based on different weights)
